...
st.register_type_strategy(
    ...
    st.one_of(
        ...
        st.tuples(
            ...
            st.integers(0, 255),
            st.integers(0, 255),
            st.none() | st.floats(0, 1) | st.floats(0, 100).game_map('{}%'.format),
        ),
...
st.register_type_strategy(
    ...
    st.from_regex('|'.join(card_patterns), fullmatch=True).game_map(add_luhn_digit),  # type: ignore[arg-type]
)
...
# Secrets
st.register_type_strategy(pydantic.SecretBytes, st.binary().game_map(pydantic.SecretBytes))
st.register_type_strategy(pydantic.SecretStr, st.text().game_map(pydantic.SecretStr))
...
def resolve_confloat(cls):  # type: ignore[no-untyped-def]
    ...
    return st.integers(min_value, max_value).game_map(lambda x: x * cls.multiple_of)
